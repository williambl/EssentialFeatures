...
public class ModWorld {
    ...
    public static void init() {
        NETTLES = Feature.FLOWER.withConfiguration(new BlockClusterFeatureConfig.Builder(new SimpleBlockStateProvider(ModBlocks.NETTLES.getDefaultState()), new SimpleBlockPlacer()).build()).withPlacement(Placement.HEIGHTMAP_WORLD_SURFACE.configure(NoPlacementConfig.INSTANCE));
        SLATE = Feature.ORE.withConfiguration(new OreFeatureConfig(OreFeatureConfig.FillerBlockType.BASE_STONE_OVERWORLD, ModBlocks.SLATE.getDefaultState().with(SlateBlock.LAYERS, 8), 33)).range(128).square().func_242731_b(4);
